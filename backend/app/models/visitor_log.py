...
class VisitorLog(BaseModel):
    ...
    visitor_request_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('visitor_requests.id'), unique=True, nullable=False)
    ...
    visitor_request: Mapped['VisitorRequest'] = relationship(back_populates='visitor_log')